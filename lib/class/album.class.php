...
class Album extends database_object
{
    ...
    public static function check($name, $year = 0, $disk = 0, $mbid = null, $band = null,  $readonly = false)
    {
        ...
        if (isset(self::$_mapcache[$name][$year][$disk][$mbid][$band])) {
            return self::$_mapcache[$name][$year][$disk][$mbid][$band];
        }
	...
	else
	{
        ...
        if ($row = Dba::fetch_assoc($db_results)) {
            $id = $row['id'];
            self::$_mapcache[$name][$year][$disk][$mbid][$band] = $id;
            return $id;
        }
        ...
        $sql = 'INSERT INTO `album` (`name`, `prefix`, `year`, `disk`, `mbid`, `band`) VALUES (?, ?, ?, ?, ?, ?)';

        $db_results = Dba::write($sql, array($name, $prefix, $year, $disk, $mbid, $band));
        if (!$db_results) {
            return null;
        }
	...
	public function update_art($new_image,$mime) {
		...
		foreach ($songs as $song_id) 
		{
			...
			if (isset($data['tags']['id3v2']))
			{
				...
				{
					debug_event('update_art', 'Nothing to do. Image is already in ID3 tag', '5');
				}
				else
				{
					if ($album_name == '')
					{
						if  ($image_from_tag <> '')
						{
							debug_event('update_art', 'Cleared Image in ID3 tag', '5');
							$id3->write_id3('');
						}
					}
					else
			...
			{
				debug_event('update_art','No id3v2','5');
			}										
		} // foreach song of album
