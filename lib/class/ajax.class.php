...
class Ajax {
	 ...
	 * textbutton
	 */
	public static function textbutton($action,$icon,$alt,$source='',$post='',$class='') {
		...
		// browsers (ie :hover actions) and also to unify linkbuttons
		// (w/o ajax) display
		$string = "<a href=\"javascript:void(0);\" id=\"$source\" $class>".$string.' '.$alt."</a>\n";
	...
	public static function image($action,$img_url,$size,$alt='',$source='',$post='',$class='') {
		...
		$string = '<img src="' . $img_url . '" width="'.$size.'" height="'.$size.'" alt="' . $alt .  '"/>';
